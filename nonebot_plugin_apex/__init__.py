...
api_key = '1e98172ec0f469bd01c59f6ae9cc4dd6'
api_url = 'https://api.mozambiquehe.re/'

player_statistics = on_command('apex玩家')
map_protation = on_command('apex地图')
predator = on_command('apex猎杀')
crafting_rotation = on_command('apex制造')
...
async def _(player_name: Message = CommandArg()):
    ...
    await player_statistics.send('正在为您正在查询...玩家 %s' % player_name)
    ...
    if response:
        if 'Error' in response.json():
            ...
            data += '大逃杀段位: %s %s \n' % (str(convert(globals.get('rank').get(
                'rankName'))), str(globals.get('rank').get('rankDiv')))
            data += '大逃杀分数: %s \n' % str(globals.get('rank').get('rankScore'))
            data += '竞技场段位: %s %s \n' % (str(convert(globals.get('arena').get(
                'rankName'))), str(globals.get('arena').get('rankDiv')))
            ...
            data += '当前状态时长: %s \n' % str(strftime('%H:%M:%S',
                                          gmtime(realtime.get('currentStateSecsAgo'))))
            await player_statistics.send(data)
    else:
        await player_statistics.send('查询失败，请稍后再试')
...
async def _():
    ...
    await map_protation.send('正在为您查询...当前地图轮换')
    response = await api_query(service, payload)
    if response:
        logger.info(response.json())
        battle_royale = response.json().get('battle_royale')
        logger.info(battle_royale)
        ...
        await map_protation.send('查询失败，请稍后再试')
...
async def _():
    ...
    await predator.send('正在为您查询...顶尖猎杀者')
    response = await api_query(service, payload)
    if response:
        ...
        await predator.send('查询失败，请稍后再试')
...
async def _():
    service = 'crafting'
    payload = {'auth': api_key}
    await crafting_rotation.send('正在为您查询...制造轮换')
    response = await api_query(service, payload)
    if response:
        data = ''
        data += '每日制造: \n'
        data += '%s %s %s 点 \n' % (str(convert(response.json()[0]['bundleContent'][0]['itemType']['name'])), str(convert(
            response.json()[0]['bundleContent'][0]['itemType']['rarity'])), str(convert(response.json()[0]['bundleContent'][0]['cost'])))
        data += '%s %s %s 点\n' % (str(convert(response.json()[0]['bundleContent'][1]['itemType']['name'])), str(convert(
            response.json()[0]['bundleContent'][1]['itemType']['rarity'])), str(convert(response.json()[0]['bundleContent'][1]['cost'])))
        data += '每周制造: \n'
        data += '%s %s %s 点\n' % (str(convert(response.json()[1]['bundleContent'][0]['itemType']['name'])), str(convert(
            response.json()[1]['bundleContent'][0]['itemType']['rarity'])), str(convert(response.json()[1]['bundleContent'][0]['cost'])))
        data += '%s %s %s 点\n' % (str(convert(response.json()[1]['bundleContent'][1]['itemType']['name'])), str(convert(
            response.json()[1]['bundleContent'][1]['itemType']['rarity'])), str(convert(response.json()[1]['bundleContent'][1]['cost'])))
        data += '赛季制造: \n'
        data += '%s %s %s 点\n' % (str(convert(response.json()[2]['bundleContent'][0]['itemType']['name'])), str(convert(
            response.json()[2]['bundleContent'][0]['itemType']['rarity'])), str(convert(response.json()[2]['bundleContent'][0]['cost'])))
        data += '%s %s %s 点\n' % (str(convert(response.json()[3]['bundleContent'][0]['itemType']['name'])), str(convert(
            response.json()[3]['bundleContent'][0]['itemType']['rarity'])), str(convert(response.json()[3]['bundleContent'][0]['cost'])))
        await crafting_rotation.send(data)
    else:
        await crafting_rotation.send('查询失败，请稍后再试')
