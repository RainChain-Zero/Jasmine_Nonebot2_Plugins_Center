...
async def video_detail(url, **kwargs):
    try:
        ...
        header = '【茉莉のbilibili链接解析】\n'
        vurl = f"https://www.bilibili.com/video/av{res['aid']}"
        title = f"\n标题：{res['title']}\n"
        cover = MessageSegment.image(
            res["pic"]) if analysis_display_image else ""
        ...
        if len(desc_list) > 4:
            desc = desc_list[0] + "\n" + \
                desc_list[1] + "\n" + desc_list[2] + "……"
        msg = Message([header, cover, vurl, title, tname, stat, desc])
        return msg, vurl
    except Exception as e:
...
async def bangumi_detail(url, time_location):
    try:
        ...
        header = '【茉莉のbilibili链接解析】\n'
        cover = MessageSegment.image(
            res["cover"]) if analysis_display_image else ""
        ...
        if time_location:
            time_location = time_location[0].replace("&amp;", "&")[3:]
            vurl += f"?t={time_location}"
        msg = Message([header, cover, f"{vurl}\n", title,
...
async def live_detail(url):
    try:
        ...
        if live_status:
            ...
            player = ""
        msg = Message([header, cover, vurl, title, up, watch, tags, player])
        return msg, vurl
    except Exception as e:
