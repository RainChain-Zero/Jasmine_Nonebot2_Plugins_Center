...
async def reply_1298754454_01(event: PrivateMessageEvent):
    if (event.sender.user_id == 1298754454):
        await trigger_1298754454_01.finish(draw_special_reply(event.sender.user_id, "...", event), reply_message=True)
...
async def reply_4786515_01(event: MessageEvent):
    if (event.sender.user_id == 4786515):
        await trigger_4786515_01.finish(draw_special_reply(event.sender.user_id, "小姐", event), reply_message=True)


@trigger_4786515_02.handle()
async def reply_4786515_02(event: MessageEvent):
    if event.sender.user_id == 4786515:
        await trigger_4786515_02.finish(draw_special_reply(event.sender.user_id, "茉莉看路灯", event), reply_message=True)


@trigger_4786515_03.handle()
async def reply_4786515_03(event: MessageEvent):
    if event.sender.user_id == 4786515:
        await trigger_4786515_03.finish(draw_special_reply(event.sender.user_id, "小姐我们回来了", event), reply_message=True)
...
async def reply_839968342_01(matcher: Matcher, bot: Bot, event: MessageEvent):
    if event.sender.user_id == 839968342:
        await matcher.finish(draw_special_reply(event.sender.user_id, "茉莉？", event), reply_message=True)
...
async def reply_3449791431_01(bot: Bot, event: MessageEvent):
    ...
    if favor > 500 and favor < 3000:
        ...
        level = 3
    await trigger_3449791431_01.finish(draw_normal_reply(level, "呜呜呜", event), reply_message=True)
