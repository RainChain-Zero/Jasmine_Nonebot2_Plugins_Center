...
class KFC:
    ...
    async def get_food(self, menu_detail: list, number: int):
        ...
        def traversal(lists):
            food_result = MessageSegment.text('========菜单========\n')
...
@KFC_eat.got("city", prompt="请输入您所在的位置，形如“城市”或“城市+空格+地区（县）”")
async def city_handler(city: str = ArgPlainText("city")):
    if '退出' in city:
        await KFC_eat.finish('')
...
@KFC_eat.got("keyword", prompt="请输入您要查询的店铺，输入关键词即可，输入句号（。）将列出部分店铺信息")
async def keyword_handler(keyword: str = ArgPlainText("keyword")):
    global Store
    if '退出' in keyword:
        await KFC_eat.finish('已退出KFC查询√')
    if '。' == keyword:
        keyword = ' '
